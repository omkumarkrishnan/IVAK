/*
  # Add Verification Methodology for DLR 5.3.1
  
  1. Purpose
    - Add comprehensive verification methodology for DLR 5.3.1 (40 percent financing disbursed in each district as per the district-level IEIAPs - cumulative)
  ...
  3. Changes
    - Update methodology field for all verifications under DLR 5.3.1
*/

-- Update methodology for DLR 5.3.1 verifications
...
The IVA will undertake site visits and interact with concerned representatives for schemes at the state, district, and village levels to verify the following:
...
WHERE dli_id = (SELECT id FROM dlis WHERE code = 'DLR 5.3.1');
